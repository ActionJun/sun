...
document.getElementById("register_local").onclick=function(){
	var obj=new Object;
	obj.name=$("user");
	obj.pwd=$("pwd");
	...
	if(localStorage.getItem("data")!==null){
		var ccstring=localStorage.getItem("data");
		var ccarr=JSON.parse(ccstring);
		ccarr.push(obj);
		var ccstr=JSON.stringify(ccarr);
		localStorage.setItem("data",ccstr);
	}else{
		var carr=[];
		carr.push(obj);
		var cstr=JSON.stringify(carr)
		localStorage.setItem("data",cstr)
	}
}